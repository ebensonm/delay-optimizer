...
def adam_optimizer_optimizer(delayer,use_delays=True,maxiter=None):
    def objective(params):
        ...
        delayer.compute_time_series(use_delays=use_delays,maxiter=maxiter)
        if (delayer.conv is True):
            return delayer.final_val
        else:
            return delayer.n * 1000
    space_search = {
        'alpha': hp.uniform('alpha', 0.9, 1.5),
        'beta_1': hp.uniform('beta_1', 0.8, 1.0),
        'beta_2': hp.uniform('beta_2', 0.8, 1.0)
    }
    best = fmin(fn = objective, space=space_search, algo=tpe.suggest, max_evals=500)
    return best['alpha'], best['beta_1'], best['beta_2']
    
def momentum_optimizer_optimizer(delayer, use_delays=True, maxiter=None):
    def objective(params):
        alpha = params['alpha']
        gamma = params['gamma']
        delayer.Optimizer.learning_rate = alpha
...
def nesterov_optimizer_optimizer(delayer, use_delays=True, maxiter=None):
    def objective(params):
        alpha = params['alpha']
        gamma = params['gamma']
        delayer.Optimizer.learning_rate = alpha
